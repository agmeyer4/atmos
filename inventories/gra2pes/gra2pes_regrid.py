...
def main():
    ...
    years = [2021]
    day_types = ['satdy']#['satdy','sundy','weekdy']
    print(f'Years: {years}')
    ...
    details_path = os.path.join(regrid_config.regridded_path,'details')
    os.makedirs(details_path,exist_ok=True)
    with open(os.path.join(details_path,'regrid_details.txt'),'w') as f:
    ...
    with open(os.path.join(details_path,'regrid_config.pkl'),'wb') as f:
        pickle.dump(regrid_config,f)
    gra2pes_regridder.save_regrid_weights(details_path)
    print('Creating example ds for gridcellarea')
    base_ds = BGH.load_fmt_fullday(sectors[0],years[0],months[0],day_types[0])
    regridded_ds = gra2pes_regridder.regrid(base_ds)
    example_ds = regridded_ds.sel(sector = sectors[0]).isel(datetime = 0).drop_vars(['sector','datetime'])[list(regridded_ds.data_vars)[0]]
    example_ds.to_netcdf(os.path.join(details_path,'grid_out.nc'))
